...
export default function ThreadView({
  ...
  const openingAtLatest = useRef(true);
  function revealLatest(grouped = false) {
    if (!openingAtLatest.current || (!grouped && mail.messages.length < 2)) return;
    const messages = scroller.current?.querySelectorAll<HTMLElement>("[data-thread-message]");
    messages?.[messages.length - 1]?.scrollIntoView({ block: "end", behavior: "auto" });
  }
    ...
    >
      ...
      <header className="message-view-header thread-view-header">
        <div className="message-column message-heading-row">
          ...
          >
            <IconButton name="Star" title={mail.starred ? "Unstar (S)" : "Star (S)"} aria-pressed={mail.starred}
              disabled={!supportsAction("star")} onClick={() => onAction("star")} />
            <IconButton
              name="Check"
              title="Mark done (E)"
      ...
      <div
        ...
        ref={scroller}
        onWheelCapture={() => { openingAtLatest.current = false; }}
        onPointerDownCapture={() => { openingAtLatest.current = false; }}
        onKeyDownCapture={() => { openingAtLatest.current = false; }}
      >
        <div
          ref={messageColumn}
                  ...
                  <>
                    ...
                    {message.loaded === false ? <div className="message-body-loading" role="status">Loading message…</div> : (
                      <MessageBody
                        ...
                        onActivate={() => { openingAtLatest.current = false; activeMessage.current = message.id; }}
                        onLayout={grouped => revealLatest(grouped)}
